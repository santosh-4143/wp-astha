...
if (!function_exists('wc_create_new_customer')) {
    ...
    function wc_create_new_customer($email, $username = '', $password = '') {
        ...
        if (isset($_FILES['prescription_upload']) && ($_FILES['prescription_upload']['size'] > 0)) {
            $arr_file_type = wp_check_filetype(basename($_FILES['prescription_upload']['name']));
            
            $uploaded_file_type = $arr_file_type['type'];
           // print_r($uploaded_file_type);
            $allowed_file_types = array('image/jpg', 'image/jpeg', 'image/gif', 'image/png' , 'application/pdf');
            if (in_array($uploaded_file_type, $allowed_file_types)) {
               // print_r($_FILES);die;
                $upload_overrides = array('test_form' => false);
                $uploaded_file = wp_handle_upload($_FILES['prescription_upload'], $upload_overrides);
                
            } else { // wrong file type
                $upload_feedback = 'Please upload only image files (jpg, gif,png or pdf).';
            }
            $customer_details['prescription_upload'] = str_replace(' ', '-', $_FILES['prescription_upload']['name']);
        }
    
        if (isset($_FILES['report_file']) && ($_FILES['report_file']['size'] > 0)) {
            $arr_file_type = wp_check_filetype(basename($_FILES['report_file']['name']));
            $uploaded_file_type = $arr_file_type['type'];
            $allowed_file_types = array('image/jpg', 'image/jpeg', 'image/gif', 'image/png' , 'application/pdf');
            if (in_array($uploaded_file_type, $allowed_file_types)) {
                $upload_overrides = array('test_form' => false);
                $uploaded_file = wp_handle_upload($_FILES['report_file'], $upload_overrides);
            } else { // wrong file type
                $upload_feedback = 'Please upload only image files (jpg, gif,png or pdf).';
            }
            $customer_details['report_file'] = str_replace(' ', '-', $_FILES['report_file']['name']);
        }
        
        $wpdb->insert('astha_user_details', $customer_details, array('%s', '%s', '%s', '%s', '%s', '%s'));
    ...
    if ($results) {
        foreach ($results as $result) {
            ...
            $download_name = apply_filters(
                    'woocommerce_downloadable_product_name', $_product->get_title() . ' &ndash; ' . $download_file['name'], $_product, $result->download_id, $file_number
            );
